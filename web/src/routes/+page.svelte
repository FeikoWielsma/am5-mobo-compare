...
<div class="container-fluid px-3 px-md-4 py-3">
  ...
  <!-- Motherboard Table with Dynamic Columns and Filter Dropdowns -->
  <div
      ...
      <table class="table table-dark table-hover table-striped align-middle mb-0 small" id="moboTable">
        <thead class="table-dark text-secondary user-select-none">
          ...
          <tr class="border-bottom border-secondary sticky-header-row-1" bind:clientHeight={row1Height}>
            ...
            <th
              class="cursor-pointer th-sortable"
              onclick={() => toggleSort('form_factor')}
              style="min-width: 105px;"
              ...
              >
                <div class="d-flex align-items-center justify-content-between gap-1">
                  ...
                  <div
                    ...
                    role="button"
                    tabindex="0"
                    onkeydown={(e) => e.key === 'Enter' && toggleSort(col.id)}
                  >
          ...
          <tr class="filter-row bg-dark-subtle border-bottom border-secondary sticky-header-row-2">
            ...
            <td>
              <ColumnFilterDropdown
                ...
                onchange={(s) => handleFilterChange('brand', s)}
                isOpen={openDropdownId === 'brand'}
                onToggle={() => (openDropdownId = openDropdownId === 'brand' ? null : 'brand')}
            ...
            <td>
              <ColumnFilterDropdown
                ...
                onchange={(s) => handleFilterChange('chipset', s)}
                isOpen={openDropdownId === 'chipset'}
                onToggle={() => (openDropdownId = openDropdownId === 'chipset' ? null : 'chipset')}
                onClose={() => (openDropdownId = null)}
              />
            </td>
            ...
            <td>
              <ColumnFilterDropdown
                ...
                onchange={(s) => handleFilterChange('form_factor', s)}
                isOpen={openDropdownId === 'form_factor'}
                onToggle={() => (openDropdownId = openDropdownId === 'form_factor' ? null : 'form_factor')}
                onClose={() => (openDropdownId = null)}
              />
            </td>
            ...
            {#each dynamicColumns as col (col.id)}
              <td>
                <ColumnFilterDropdown
                  ...
                  onchange={(s) => handleFilterChange(col.id, s)}
                  isOpen={openDropdownId === col.id}
                  onToggle={() => (openDropdownId = openDropdownId === col.id ? null : col.id)}
                  onClose={() => (openDropdownId = null)}
                />
              </td>
            {/each}
        ...
        <tbody>
          {#if filteredBoards.length === 0}
            <tr>
              <td colspan={7 + dynamicColumns.length} class="text-center py-5 text-secondary">
